...
const SearchResultsCard = ({ hotel }: Props) => {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[2fr_3fr] custom-shadow-rounded animate-slideIn p-3 md:p-8 gap-8">
      ...
      <div className="grid gap-3">
          ...
          <div className="flex items-center">
            <span className="flex">
              {Array.from({ length: hotel.starRating }).map((_, index) => (
                <AiFillStar className="fill-yellow-400 w-5 h-5" key={index} />
              ))}
            </span>
        ...
        <div>
          <div className="line-clamp-4 text-md sm:text-lg">{hotel.description}</div>
        </div>
        ...
        <div className="grid grid-rows-2 grid-cols-1 gap-4 md:gap-0 md:grid-rows-1 md:grid-cols-2 items-end whitespace-nowrap">
          <div className="flex gap-1 items-center">
            {hotel.facilities.slice(0, 3).map((facility) => (
              <span
                className="bg-slate-300 px-2 py-1 rounded-lg font-semibold text-sm sm:text-lg whitespace-nowrap"
                key={facility}>
                {facility}
              </span>
            ))}
